...
def get_document_types():
    ...
    url = "https://pericom.epik.live/api/v4/document_types/"
    document_types = []
    next_url = url
    while next_url:
        response = requests.get(next_url, auth=('admin', 'jJHPeUMJd9u5$hRuH9sj'))
...
def get_metadata_types(doc_type_id):
    ...
    url = f"https://pericom.epik.live/api/v4/document_types/{doc_type_id}/metadata_types/"
    response = requests.get(url, auth=('admin', 'jJHPeUMJd9u5$hRuH9sj'))
    if response.status_code == 200:
